  ...
  return (
    <div className="mt-3 rounded-[12px] border border-[#E5E0D4] bg-[#FCFAF5]">
      ...
      >
        ...
        <span className="rounded-full bg-[#EDEAFF] px-1.5 py-0.5 text-[10px] font-semibold text-[#5849F2]">{reasoning.length}</span>
      </button>
      {isOpen ? (
        <div className="border-t border-[#E5E0D4] px-4 py-3">
          <ul className="space-y-1.5">
            {reasoning.map((item, index) => (
              <li key={`${item}-${index}`} className="text-[13px] leading-[1.6] text-[#7B7E8F]">
                {item}
              </li>
            ))}
          </ul>
