...
import type { ChatChartConfig, ChatQueryResult } from '@/components/chat/use-chat-stream'
...
const CHART_COLORS = ['#5849F2', '#7B7E8F', '#C2CBD4', '#EDEAFF', '#D8D2FF', '#F3EEE3']

function renderChart(config: ChatChartConfig, rows: Record<string, unknown>[], xKey: string, yKey: string) {
    ...
    case 'bar':
      return (
        <BarChart data={rows}>
          ...
          <Tooltip contentStyle={{ borderRadius: 12, border: '1px solid #E5E0D4', fontSize: 12 }} />
    ...
    case 'area':
      return (
        <AreaChart data={rows}>
          ...
          <Tooltip contentStyle={{ borderRadius: 12, border: '1px solid #E5E0D4', fontSize: 12 }} />
          {series.map((key, index) => (
            <Area key={key} type="monotone" dataKey={key} stroke={CHART_COLORS[index % CHART_COLORS.length]} fill={CHART_COLORS[index % CHART_COLORS.length]} fillOpacity={0.15} />
          ))}
        </AreaChart>
      )
    case 'pie':
      return (
        <PieChart>
          <Pie data={rows} dataKey={yKey} nameKey={xKey} cx="50%" cy="50%" outerRadius={80} label={({ name }) => String(name)} labelLine={false}>
            {rows.map((_, index) => (
...
export function ChatChart({ config, data }: ChatChartProps) {
  ...
  if (config.type === 'metric_card') {
    return (
      <div className="mt-3 flex items-center gap-4 rounded-[16px] border border-[#E5E0D4] bg-[#FCFAF5] p-6">
        <div>
          <p className="text-[12px] font-semibold uppercase tracking-[0.2em] text-[#7B7E8F]">{config.label ?? config.title ?? 'Metric'}</p>
  ...
  return (
    <div className="mt-3 overflow-hidden rounded-[16px] border border-[#E5E0D4] bg-white p-4">
      {config.title ? <p className="mb-3 text-[13px] font-semibold text-[#313852]">{config.title}</p> : null}
